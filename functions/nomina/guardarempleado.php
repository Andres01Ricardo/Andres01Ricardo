...
$oItem=new Data("tercero","nit",$datos["numeroDocumento"]); 
$aValidateTercero=$oItem->getDatos(); 
unset($oItem); 

if(empty($aValidateTercero)){

    if(!isset($_SESSION)){ session_start(); }

    $aDatos["tipoPersona"]=1; 
    $aDatos["nit"]=$datos["numeroDocumento"]; 
    $aDatos["digitoVerificador"]=0; 
    $aDatos["razonSocial"]=$datos["nombres"].' '.$datos["apellidos"]; 
    $aDatos["email"]=$datos["correo"];;
    $aDatos["telefono"]=$datos["telefono"];
    $aDatos["idDepartamento"]=$datos["idDepartamentoResidencia"]; 
    $aDatos["idCiudad"]=$datos["idCiudadResidencia"]; 
    $aDatos["idPais"]=42;
...
if(!empty($aValidateTercero)){
    $oLista=new Lista("tercero_empresa");
    $oLista->setFiltro("idTercero","=",$aValidateTercero["idTercero"]);
    $oLista->setFiltro("idEmpresa","=",$_SESSION["idEmpresa"]);
...
L-1-2111016 , L-1-2111017 , L-1-2111018